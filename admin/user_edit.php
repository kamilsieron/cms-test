...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	...
	if (!empty($_FILES['avatar']['tmp_name'])) {
		...
		if (move_uploaded_file($_FILES['avatar']['tmp_name'], $targetPath)) {
			$avatarPath = 'public/avatars/' . $newName;
		}
	}

if ($avatarPath && $username) {
    $userModel->updateAvatar($username, $avatarPath);
    $_SESSION['user']['avatar'] = $avatarPath; // odśwież awatar w sesji, jeśli edytujesz siebie
}	
...
$avatarPath = !empty($user['avatar'])
    ? '/' . $user['avatar']
    : '/public/avatars/default.png'; // plik default.png musisz wcześniej dodać
?>
    <div class="mb-3">
        <label class="form-label">Awatar (JPG lub PNG)</label><br>
        <img src="<?= BASE_URL . '/' .$avatarPath ?>" alt="Awatar" class="img-thumbnail" style="max-width: 150px;">
    </div>
